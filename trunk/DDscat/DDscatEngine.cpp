...
DDscatEngine::DDscatEngine(void)
{
	cflpar_default = string("ddscat.par");
	strcpy(DDscatLabel, "DDscat");
...
void DDscatEngine::SetParameterFile(const char *arg)
{
	if (arg)
		cflpar = string(cflpar_default);
	else
		cflpar = cflpar_default;
}

void DDscatEngine::SayHello(void)
{
	sprintf(Message, " using parameter file=%s", cflpar.c_str());
	Wrimsg(DDscatLabel, Message);
	if (nrfld == NearfieldMethodNo)
		sprintf(Message, "to do calculation of polarizations");
	else
		sprintf(Message, "use calculated polarization to do nearfield calculations");
	Wrimsg(DDscatLabel, Message);
}

void DDscatEngine::SayBye(void)
{
	if (nrfld == NearfieldMethodNo)
		sprintf(Message, "normal termination, with no nearfield calculation");
	else
		sprintf(Message, "normal termination, after nearfield calculation");
	Wrimsg(DDscatLabel, Message);
}

// =>
void DDscatEngine::Run(void)
{
...
** */
	...
	manager = TargetManager::GetInstance();
	manager->Ioshp() = true;
	AbstractTarget *currentTarget = manager->GetCurrentTarget();
	paramDDscat = DDscatParameters::GetInstance();
...
** */
	...
	int file22, file23, paramMxnx, paramMxny, paramMxnz;
#ifdef _WIN32
	...
	{
		int res = paramDDscat->Load(cflpar.c_str(), nrfld);
		if (res > 0)
		{
...
//ChB			Vect3<int> shiftt = currentTarget->ForceMinmax(jMinmax);
//
...
////int jzz = temp.data[2];
				...
				temp.data[1] += delz[1];
				temp.data[2] += delz[2];
				currentTarget->Ixyz().Fill3(i, temp.data[0] - 1, temp.data[1] - 1, temp.data[2] - 1);
				absoluteAddresses[i] = currentTarget->GetRelativeLinearAddress(temp.data[0], temp.data[1], temp.data[2]);
//					1 + currentTarget->GetMinJx() - jMinmax.data[0], 1 + currentTarget->GetMinJy() - jMinmax.data[2], 1 + currentTarget->GetMinJz() - jMinmax.data[4]);
///if (i < 32)
///fprintf(stderr, "%6d %6d %4d %4d %4d %4d %4d %4d\n", i, absoluteAddresses[i], temp.data[0], temp.data[1], temp.data[2], jxx, jyy, jzz);
			}
///fprintf(stderr, "\n\n");
...
// Instantiate Output manager
	outputer = OutputManager::GetInstance();
...
// May change this in future.
		greener = GreenFunctionManager::GetInstance();
		switch(manager->Jpbc())
		{
		case PeriodicNo:
			greener->SetIpbc(false);
			break;
		...
		greener->SetDimension(currentTarget->Nx(), currentTarget->Ny(), currentTarget->Nz());
		greener->AllocateElectricBuffer();
//
...
// ChB 7.2.1. nambient is a parameter
	Share1(currentTarget, paramDDscat, dielec->GetFileNames(), daeff, ensc_lf, em1_lf, em2_lf, manager->Ioshp(), greener->Ipbc(), 
